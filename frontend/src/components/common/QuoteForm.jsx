...
/* Input styling uses CSS vars so it adapts to both themes */
const inputBase =
...
function Field({ id, label, value, onChange, type = "text", testid }) {
  return (
    <div className="relative">
      <input
        id={id} data-testid={testid} type={type} value={value}
        onChange={onChange} placeholder={label} className={inputBase}
      />
  ...
  return (
    <form data-testid="quote-form" onSubmit={handleSubmit} className="space-y-5">
      <div className={`grid gap-5 ${compact ? "" : "sm:grid-cols-2"}`}>
        ...
        <div className="relative">
          <select
            id="qf-pt" data-testid="qf-productType"
            value={data.productType} onChange={set("productType")}
            className="w-full appearance-none rounded-[2px] border-b border-border-soft bg-surface-floating px-3 pb-2 pt-6 text-[15px] text-ink focus:border-gold focus:outline-none"
          >
      ...
      <label className="flex items-center gap-3 text-sm text-ink/60 cursor-pointer">
        <input
      ...
      <button
        data-testid="qf-submit" type="submit" disabled={loading || sent}
