...
export default function WhyChooseUs() {
  ...
  return (
    <section data-testid="why-section" className="relative overflow-hidden bg-surface-primary py-28">
      <span className="display pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 select-none whitespace-nowrap text-[18vw] leading-none text-ink/[0.03]">
        EXCELLENCE
      </span>
        ...
        <div ref={gridRef} className="grid gap-x-10 gap-y-14 sm:grid-cols-2 lg:grid-cols-4">
          {WHY_CHOOSE.map((f) => (
            <div key={f.num} data-feat className="group">
              <span className="display block text-6xl font-light leading-none text-ink/[0.06] transition-colors duration-300 group-hover:text-ink/[0.12] lg:text-[80px]">
                {f.num}
              </span>
