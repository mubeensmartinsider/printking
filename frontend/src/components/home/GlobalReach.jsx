...
export default function GlobalReach() {
  ...
  return (
    <section data-testid="global-section" className="relative bg-surface-base py-28 overflow-hidden">
      <div className="section-pad mx-auto max-w-[1400px]">
        ...
        <div
          ref={mapRef}
          className="relative mx-auto aspect-[2/1] w-full max-w-5xl overflow-hidden border border-border-soft bg-surface-elevated"
          style={{
            backgroundImage: "radial-gradient(var(--color-border) 1px, transparent 1.4px)",
            backgroundSize: "18px 18px",
          }}
        >
          <svg viewBox="0 0 100 50" preserveAspectRatio="none" className="absolute inset-0 h-full w-full">
            {GLOBAL_REACH.regions.map((r) => (
              <path key={r.name} d={arc(ORIGIN, { x: r.x, y: r.y / 2 })}
                fill="none" stroke="rgba(197,160,90,0.35)" strokeWidth="0.26" strokeDasharray="1 1" />
            ))}
          </svg>
