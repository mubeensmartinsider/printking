    ...
    const ctx = gsap.context(() => {
      ...
      cols?.forEach((col, i) => {
        gsap.to(col, {
          yPercent: i % 2 === 0 ? -8 : 8,
          ease: "none",
          scrollTrigger: { trigger: gridRef.current, start: "top bottom", end: "bottom top", scrub: 1 },
        });
      });
    }, gridRef);
    return () => ctx.revert();
      ...
      <div className="section-pad mx-auto grid max-w-[1400px] gap-16 lg:grid-cols-2 lg:gap-24">
        ...
        <div ref={gridRef} className="grid grid-cols-2 gap-4">
          {ABOUT.images.map((img, i) => (
            <div key={i} data-parallax className={`group relative overflow-hidden ${i % 2 === 0 ? "mt-0" : "mt-8"}`}>
              <Img
                src={img.src}
                alt={img.caption}
                label={img.caption}
